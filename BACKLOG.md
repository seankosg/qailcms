...
## TM TC 두 열 성능 (2026-08-12, 종결)
- [보류] tc 비용은 전일 `tm_row_tplan` 호출 ~347ms (main 279행 135ms / sub 1,796행 211ms).
  - sub 를 `tm_kpi_tplan` 직접 호출로 바꾸면 ~200ms 절감 가능하나, `tm_row_tplan` 의 main/sub 분기를 밖에 복제하게 되므로 **하지 않는다**.
