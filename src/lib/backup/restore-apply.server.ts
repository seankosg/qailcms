 ...
 *  - 통신 오류·타임아웃·응답 유실은 실패(롤백)로 단정하지 않고 미확정으로 반환한다.
 *
 * 안전 스냅샷의 시점 한계(중요):
 *  이 스냅샷은 표를 순차로 읽는 논리 사본이라 표 간 단일 시점 일관성이 없고,
 *  스냅샷 종료와 반영 트랜잭션 시작 사이의 쓰기는 어떤 사본에도 담기지 않는다.
 *  따라서 "복원 직전 운영 상태의 완전한 보증"이 아니다. 자세한 내용과 HP4 전 판단사항은
 *  docs/backup-restore.md 「안전 스냅샷의 시점 한계」 참고.
 *
