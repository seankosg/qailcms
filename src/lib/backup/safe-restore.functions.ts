...
export const getRestoreRunStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    return {
      ...
      preflight_summary: {
        blockers: (run.preflight_result?.blockers ?? []) as unknown[],
        warnings: (run.preflight_result?.warnings ?? []) as unknown[],
        manifest_sha256: (run.manifest_sha256 ?? null) as string | null,
      },
      staging_verify: (run.staging_verify ?? null) as unknown,
      staging_overall_digest: (run.staging_overall_digest ?? null) as string | null,
      apply_result: (run.apply_result ?? null) as unknown,
