...
import {
  ...
  type TaskManagementFieldConfigRow,
} from "@/hooks/useTaskManagementFieldConfig";
import { TM_COLUMNS, GROUP_HEADER_BG } from "@/lib/task-management/columns";
import { useCurrentUser } from "@/hooks/useCurrentUser";

type DraftMap = Record<string, Partial<Pick<TaskManagementFieldConfigRow, "display_name" | "is_visible" | "sort_order" | "note" | "group_key">>>;

const GROUP_OPTIONS = Object.keys(GROUP_HEADER_BG);

export function TmFieldConfigTable() {
  const { data: rows = [], isLoading, refetch } = useTaskManagementFieldConfig();
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <CardTitle className="text-base">Field Config — Task Raw Data 컬럼 헤더</CardTitle>
          <p className="text-xs text-muted-foreground mt-1">
            Task Management Raw Data 표의 컬럼 라벨/기본 노출/기본 정렬을 편집합니다.
          </p>
        </div>
