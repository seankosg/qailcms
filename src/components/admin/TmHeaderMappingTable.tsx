...
import {
  useTaskManagementHeaderMappings,
  TASK_MANAGEMENT_HEADER_MAPPING_QK,
...
export function TmHeaderMappingTable() {
  ...
  const { data: fields = [] } = useTaskManagementFieldConfig();
  ...
  const filtered = useMemo(() => {
    ...
    if (!s) return rows;
    return rows.filter(
      (r) => r.source_header.toLowerCase().includes(s) || r.target_field.toLowerCase().includes(s),
    );
  }, [rows, search]);
  ...
  const saveTargetField = async (r: TaskManagementHeaderMappingRow, next: string) => {
    const { error } = await (supabase as any)
      .from("task_management_header_mappings")
  ...
  const removeRow = async (r: TaskManagementHeaderMappingRow) => {
    if (!canEdit) return;
  ...
  const submitNew = async () => {
    ...
    const { error } = await (supabase as any).from("task_management_header_mappings").insert({
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <CardTitle className="text-base">Header Mapping — Task Excel Import 별칭</CardTitle>
          <p className="text-xs text-muted-foreground mt-1">
            Task Management 업로드용 Excel 원본 헤더를 시스템 필드에 연결합니다. 시스템 매핑은 잠금 상태입니다.
          </p>
        </div>
        {canEdit && (
          ...
          <Table>
            ...
            <TableBody>
              ...
              {!isLoading && filtered.length === 0 && (
                <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground">매핑이 없습니다.</TableCell></TableRow>
              )}
              {filtered.map((r) => (
