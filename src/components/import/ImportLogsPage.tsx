  ...
  const fetchBatches = async () => {
    ...
    if (kind === "spare_part") {
      ...
      const { data } = await (supabase as any)
        ...
        .select(
          "id, file_name, status, started_at, finished_at, imported_by, data_date, total_rows, inserted, updated, inactivated, mismatched, skipped_no_key, team, rolled_back_at",
        )
        .order("started_at", { ascending: false })
        .limit(100);
      const list: Batch[] = (data ?? []).map((r: any) => ({
