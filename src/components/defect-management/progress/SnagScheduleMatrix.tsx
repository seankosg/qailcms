...
// 스티키 컬럼/헤더는 완전 불투명. 배경 뒤 컬럼이 절대 비치지 않도록 이중 스택 gradient.
...
export function SnagScheduleMatrix({
  data,
  bucket,
  stagesToShow,
  today,
  ...
  const totalBlockTitle = isMultiStage
    ? `${stagesToShow.map((s) => STAGE_LABELS[s]).join(" + ")} progress / (defects × ${stagesToShow.length})`
    : `${stageLabel} progress / defects`;
                        ...
                        >
                          <span
                            className={cn(
                              ...
                              st === "rectified" && "bg-primary/20 text-primary",
                              st === "closure" && "bg-emerald-500/15 text-emerald-700 dark:text-emerald-400",
                            )}
                          >
                            {STAGE_LABELS[st]}
                          </span>
