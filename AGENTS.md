...
절차 위반 시 롤백하고 별도 UI 변경 티켓으로 분리한다. Snag Progress 합계 호출은 실제 사용 `stages`를 `queryKey`와 RPC에 전달해 미선택 단계 계산만 생략하며 판정식은 유지한다.
...
**합계 = 모집단 검산 (필수, 2026-08-01 추가)**: 판정·집계 패널을 만들 때는 "합계 = 모집단" 검산을 화면 단위 자체 점검으로 포함할 것. 저장 스냅샷 컬럼은 값이 비어 있어도 에러 없이 숫자만 작아지므로(2026-08-01 자동 판정 분포 172 vs 모집단 1,471), 분포·도넛·스택 차트는 미분류/누락 버킷을 명시적으로 만들어 총합이 대상 행 수와 일치하는지 표시·검증한다.

### 임포트 파서·권한·로그 규칙 (필수, 2026-08-05)

- 임포트 파서는 헤더를 추정하지 않는다. 헤더 텍스트로 못 찾으면 **미매핑**으로 두고 그 컬럼을 임포트에서 제외한다. 위치·순서·인덱스로 컬럼을 가정하는 코드를 새로 만들지 않는다.
- 매핑이 부족하면 **별칭**으로 푼다(`/admin/mapping` 등록 포함). 추측 별칭을 코드에 임의 추가하지 않는다.
- 값의 형태가 필드 정의와 다르면 헤더가 맞아도 **미매핑으로 강등**한다(임계 80%, `task_no` 는 90% 미만 시 파싱 중단). 강등 표시에는 실측 비율·모집단·표본 3건을 함께 낸다.
- 미매핑·강등은 접지 않고 **전부 노출**하며, 사용자가 "이 컬럼들 없이 진행"을 명시적으로 승인해야 Start 가 활성화된다. 그 선택은 임포트 로그에 남긴다.
- 임포트 권한 판정은 역할 이름을 하드코딩하지 않는다. **`rcl_grants(모듈,'import')` 가 유일한 근거**다.
- 행이 제외되면 사유별로 세어 **화면과 로그 양쪽**에 남긴다. 항등식(파싱 = 반영 + 권한제외 + 스코프제외 + 중복 + 거부 + 정책스킵)이 안 맞으면 **미분류**로 노출한다.
- 임포트 `status` 는 **반영 행수 = 파싱 행수** 일 때만 `success`. 반영 0 이면 `failed`, 그 외 제외·거부가 하나라도 있으면 `partial`.
- 어드민 화면에서 바꾼 설정(RCL 격자 등)은 마이그레이션에 남지 않는다. 확정된 값은 **멱등 마이그레이션**으로 고정한다.
