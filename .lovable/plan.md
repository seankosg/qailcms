...
레퍼런스의 구조·좌표 계산은 유지하되, 3줄 나열에서 정보가 뭉개지지 않도록 다음을 적용합니다.

- **Plot 줄 구분**: 각 Plot 줄을 `rounded-md border bg-muted/30` 블록으로 감싸고, 좌측에 `Plot C` 라벨 배지 + 우측에 해당 Plot의 D-Day 배지를 한 줄 헤더로 배치. 줄 사이 간격 `gap-4`.
- **오늘 위치 대비**: 경과선은 `destructive`(빨강), 미경과 구간은 `border` 회색으로 굵기 3px 유지. Today 마커는 세로 점선 + 날짜 배지로 항상 눈에 띄게.
- **상태 색 대비**: 완료 = `success` 채움 + 체크 아이콘, 진행중(다음 도래) = `primary` 링 강조 + 살짝 확대(h-10) + 시계 아이콘, 예정 = `muted` 테두리 회색 원 + 빈 원 아이콘. 진행중 노드만 굵은 텍스트로 강조.
- **텍스트 위계**: 마일스톤명(굵게) > D-Day(모노 대형, 색상 구분: 미래 `primary` / 오늘 `warning` / 경과 `muted-foreground`) > 기준일(작은 모노 회색). 경과한 노드는 전체 `opacity-70`으로 낮춰 남은 일정이 부각되게.
- **가독 폭 확보**: 가로 스크롤(`min-w-[720px]`) 유지, 노드 라벨 겹침 방지를 위해 첫/마지막 노드 정렬 보정 유지.
- **다크모드**: 모든 색은 시맨틱 토큰(`success`/`warning`/`destructive`/`primary`/`muted`)만 사용하여 라이트·다크 양쪽 대비 확보. 하드코딩 색상 미사용.
