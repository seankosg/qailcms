...
begin
  ...
  if _kpi_mode is not null and length(trim(_kpi_mode)) > 0 then
    _where_sql := _where_sql || format(
      ' and public.tm_kpi_bucket_matches(%L, actual_progress, actual_finish, actual_start, plan_start, plan_end, plan_days, plan_progress, %L::date, 0.05::numeric, %s::numeric)',
...
CREATE OR REPLACE FUNCTION public.tm_items_search_ids(_q text DEFAULT NULL::text, _filters jsonb DEFAULT '[]'::jsonb, _include_inactive boolean DEFAULT false, _limit integer DEFAULT 100000, _kpi_mode text DEFAULT NULL::text, _as_of date DEFAULT NULL::date, _thresholds jsonb DEFAULT NULL::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
 SET search_path TO 'public'
