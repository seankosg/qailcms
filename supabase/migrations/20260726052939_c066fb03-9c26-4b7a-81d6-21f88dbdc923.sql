...
DECLARE
  _allowed_cols constant text[] := ARRAY[
    ...
    'latest_rev','latest_status','approval_date','status_group','is_active','is_excluded','field_mismatch','data_date','updated_at','created_at'
  ];
...
BEGIN
  ...
  IF _excluded_mode = 'only' THEN
    _where := _where || ' and coalesce(is_excluded, false) = true';
  ELSIF _excluded_mode = 'all' THEN
    NULL;
  ELSE
    _where := _where || ' and coalesce(is_excluded, false) = false';
  END IF;
