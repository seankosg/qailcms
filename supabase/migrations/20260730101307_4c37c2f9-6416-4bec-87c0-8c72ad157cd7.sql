...
BEGIN
  ...
  IF v_past AND (COALESCE(_row.is_terminated,false) OR (v_norm = 'A' AND _row.approval_date IS NULL)) THEN
...
AS $function$
  ...
  -- NO_HISTORY(과거 as-of 복원 불가) 는 별도 키로 분리.
  SELECT CASE
...
-- As-of 시점 판정이 반영된 행 소스(정본). 오늘/미래 미지정이면 저장 컬럼 그대로(빠른 경로).
...
BEGIN
  ...
  ELSE
    RETURN QUERY
      SELECT jsonb_populate_record(r.*, public.abd_judge_v1(r.*, _as_of))
      FROM public.abd_items_raw r;
  END IF;
END;
$function$;

GRANT EXECUTE ON FUNCTION public.abd_rows_as_of(date) TO authenticated, service_role;