...
RETURNS text[] LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT (SELECT array_agg(t) FROM unnest(
    (SELECT coalesce(x, '{}'::text[]) FROM (SELECT NULL::text[] x) s)
  ) t) ;
$$;

-- 기존 정의를 보존하며 SPL 신규 테이블만 추가
CREATE OR REPLACE FUNCTION public.get_backup_tables()
RETURNS text[] LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT ARRAY[
    ...
    'spl_documents','spl_document_item_links','spl_ocs_comment_document_links'
  ]::text[];
$$;
...
DECLARE b text;
BEGIN
  FOREACH b IN ARRAY ARRAY['spl-ocs-attachments','spl-ocs-source-files','spl-documents'] LOOP
    EXECUTE format($f$CREATE POLICY %I ON storage.objects FOR SELECT TO authenticated USING (bucket_id = %L)$f$, 'spl_stor_sel_'||replace(b,'-','_'), b);
    EXECUTE format($f$CREATE POLICY %I ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = %L AND public.spl_ocs_can_manage())$f$, 'spl_stor_ins_'||replace(b,'-','_'), b);
    EXECUTE format($f$CREATE POLICY %I ON storage.objects FOR UPDATE TO authenticated USING (bucket_id = %L AND public.spl_ocs_can_manage()) WITH CHECK (bucket_id = %L AND public.spl_ocs_can_manage())$f$, 'spl_stor_upd_'||replace(b,'-','_'), b, b);
    EXECUTE format($f$CREATE POLICY %I ON storage.objects FOR DELETE TO authenticated USING (bucket_id = %L AND public.spl_ocs_can_manage())$f$, 'spl_stor_del_'||replace(b,'-','_'), b);
  END LOOP;
END $do$;