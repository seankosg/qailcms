DROP FUNCTION IF EXISTS public.abd_progress_totals_json(text[],text[],text[],date,text,text);
DROP FUNCTION IF EXISTS public.abd_progress_totals(text[],text[],text[],date,text,text);
...
AS $function$
  ...
  WITH selected_stages(stage) AS MATERIALIZED (
    SELECT s.stage
    FROM (VALUES ('draft_start'),('draft_finish'),('submission'),('dar'),('approval')) AS s(stage)
    WHERE _stages IS NULL OR cardinality(_stages) = 0 OR s.stage = ANY(_stages)
  ),
  ...
  ev AS MATERIALIZED (
    ...
    JOIN selected_stages s ON s.stage = e.stage
  ),
  tot AS (
    SELECT b.gk, s.stage,
    ...
    FROM base b CROSS JOIN selected_stages s
    GROUP BY 1,2
  ),
  cnt AS (
    SELECT b.gk, e.stage,
...
AS $function$
  ...
  FROM public.abd_progress_totals(
    _plots, _teams, _group_by, _as_of_date, _plan_mode, _round, _stages
  ) t;
$function$;
