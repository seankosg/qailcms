...
AS $function$
  -- 계획(Plan)은 As-of 무관 정본(judge_now = 최종 시점 판정)으로 산출한다.
  -- 실적(Actual) 및 'current' plan_mode 의 잔여계획만 _as_of_date 판정을 쓴다.
  WITH base AS (
    SELECT (public.abd_judge_v1(r, _as_of_date)) AS judge,
           (public.abd_judge_v1(r, CURRENT_DATE)) AS judge_now,
           r.*
  ...
  expanded AS (
    SELECT b.id AS item_id,
           ...
           (b.judge_now->>'active_round')::int AS v_active_now,
  ...
  docs AS (
    SELECT b.id AS item_id,
           ...
           CASE
             ...
             WHEN (b.judge_now->>'bucket_top') = 'Approved' THEN
               COALESCE(
                 CASE
             ...
             ELSE COALESCE(
                    CASE (b.judge_now->>'active_round')::int
  ...
  WHERE pdate IS NOT NULL
    ...
    AND (
      (_plan_mode = 'baseline' AND rn <= v_active_now)
