...
-- Base status_group column values (approved / in_progress / not_started)
IF _status_group IN ('approved','in_progress','not_started') THEN
  _where := _where || format(' and status_group = %L', _status_group);
-- Dashboard row1 buckets: under_review -> bucket_top='UR', drafting -> bucket_top='DS'
...
-- Dashboard row2 delay buckets (delay_bucket text[], excluding approved rows)
ELSIF _status_group IN ('rs_delay','sb_delay','ds_delay') THEN
  _where := _where || format(
    ' and latest_status_norm is distinct from ''A'' and %L = any(coalesce(delay_bucket, ''{}''::text[]))',
    upper(_status_group)
  );
ELSIF _status_group = 'no_plan' THEN
  _where := _where || ' and latest_status_norm is distinct from ''A'' and needs_planning = true';
ELSIF _status_group = 'delayed' THEN
  _where := _where || ' and latest_status_norm is distinct from ''A'' and ('
    || 'coalesce(needs_planning, false) = true'
    || ' or ''RS_DELAY'' = any(coalesce(delay_bucket, ''{}''::text[]))'
    || ' or ''SB_DELAY'' = any(coalesce(delay_bucket, ''{}''::text[]))'
    || ' or ''DS_DELAY'' = any(coalesce(delay_bucket, ''{}''::text[]))'
    || ')';
END IF;
