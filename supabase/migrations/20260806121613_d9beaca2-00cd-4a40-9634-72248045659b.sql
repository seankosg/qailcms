CREATE OR REPLACE VIEW public.v_task_management_raw_derived AS
 WITH cfg AS (
         ...
         SELECT DISTINCT ON (r.main_task_no) r.main_task_no, r.milestone, c3.target_date
           FROM task_management_raw r
           JOIN cfg c3 ON c3.plot = r.plot AND c3.kind = r.milestone
          WHERE r.level = 'sub' AND r.is_active AND r.main_task_no IS NOT NULL AND r.milestone IS NOT NULL
          ORDER BY r.main_task_no, c3.target_date DESC
        )
 SELECT t.id,
    t.task_no,
    t.main_task_no,
    ...
    t.delay_days,
    t.alarm_reason,
    CASE WHEN t.level = 'main'::text AND sm.main_task_no IS NOT NULL THEN sm.milestone ELSE t.milestone END AS milestone,
    CASE WHEN t.level = 'main'::text AND sm.main_task_no IS NOT NULL THEN sm.target_date ELSE c.target_date END AS milestone_date,
        CASE
   ...
   FROM task_management_raw t
     ...
     LEFT JOIN sub_ms sm ON t.level = 'main'::text AND sm.main_task_no = t.task_no;