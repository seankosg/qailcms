...
BEGIN
  ...
  -- 사용자 Compliance 안전 승계: complied=true 만, 자식에 행 없을 때만 (부모 행/로그 물리 보존)
  WITH carry AS (
  ...
  -- Contractor Response 구간 보존 (unique: parent, seg_no, source_hash)
  WITH seg AS (
  ...
  -- confirmed_high 만 개별 atomic 연결 (unique: response_segment_id, atomic_comment_id)
  WITH lk AS (
...
-- dryrun: 교정 로그 실제 칸 이름(ocs_after) 반영
