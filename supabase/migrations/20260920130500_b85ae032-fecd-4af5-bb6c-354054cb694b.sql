-- 2026-09-20 (2): 지적 5·6·7 정리
--  5) 합계 완료 판정은 정본 _snag_done_asof 만 사용(1차 마이그레이션에서 적용 완료)
--  6) done_upto(완료 수)와 actual_upto(실적 누계)를 별도 식으로 산출 → 사본 공유 제거
--  7) 서버 기본 _plan_mode 를 화면 기본값과 동일하게 'remaining' 으로 정렬

CREATE OR REPLACE FUNCTION public.snag_progress_events(_as_of_date date, _plan_mode text DEFAULT 'remaining'::text)
...
CREATE OR REPLACE FUNCTION public.snag_progress_cell_ids(_stage text, _field text, _from date, _to date, _as_of date DEFAULT NULL::date, _plan_mode text DEFAULT 'remaining'::text)
...
AS $function$
  ...
  -- _plan_mode='remaining' + _field='planned' + _from <= '0001-01-01' → "잔여 전체" 계약.
  --   계획일 유무와 무관하게 해당 스테이지가 as-of 기준 미완료인 항목 전부. 판정은 정본 _snag_done_asof.
  SELECT r.id
  FROM public.defect_items_raw r
  CROSS JOIN LATERAL (VALUES
...
CREATE OR REPLACE FUNCTION public.defect_snag_progress_totals(_plan_groups text[], _teams text[], _room_groups text[], _group_by text[], _as_of_date date, _plan_mode text DEFAULT 'remaining'::text, _buildings text[] DEFAULT NULL::text[])
...
AS $function$
  ...
  flags AS (
    SELECT gk, psd, pcd, ppd, pdd, pxd, phd,
      ...
      -- 실적 누계(actual_upto): 실적일 ≤ 기준일. 완료 수와 별도 식으로 유지(둘을 공유하지 않는다)
      (asd  IS NOT NULL AND asd  <= _as_of_date) AS s_act,
      (acd  IS NOT NULL AND acd  <= _as_of_date) AS r_act,
      (apd  IS NOT NULL AND apd  <= _as_of_date) AS p_act,
      (add_ IS NOT NULL AND add_ <= _as_of_date) AS d_act,
      (axd  IS NOT NULL AND axd  <= _as_of_date) AS c_act,
      (ahd  IS NOT NULL AND ahd  <= _as_of_date) AS h_act,
  ...
  agg AS (
    SELECT gk,
      ...
      count(*) FILTER (WHERE s_act)::int AS sac,
      count(*) FILTER (WHERE r_act)::int AS rac,
      count(*) FILTER (WHERE p_act)::int AS pac,
      count(*) FILTER (WHERE d_act)::int AS dac,
      count(*) FILTER (WHERE c_act)::int AS cac,
      count(*) FILTER (WHERE h_act)::int AS hac,
      -- remaining = 이동형: 완료된 단계는 계획을 실제 완료일(≤ as-of)로 옮겨 계상
  ...
  SELECT a.gk, s.stage, a.total, s.done_cnt, s.plan_cnt, s.act_cnt, s.np_cnt,
         COALESCE(mk.np_mask, '{}'::jsonb)
  FROM agg a
  LEFT JOIN masks mk ON mk.gk IS NOT DISTINCT FROM a.gk
  CROSS JOIN LATERAL (VALUES
    ('start'::text,     a.sdc, a.sp, a.sac, a.snp),
    ('rectified',       a.rdc, a.rp, a.rac, a.rnp),
    ('pre_inspection',  a.pdc, a.pp, a.pac, a.pnp),
    ('dar_inspection',  a.ddc, a.dp, a.dac, a.dnp),
    ('closure',         a.cdc, a.cp, a.cac, a.cnp),
    ('ho',              a.hdc, a.hp, a.hac, a.hnp)
  ) AS s(stage, done_cnt, plan_cnt, act_cnt, np_cnt)
$function$;