-- ABD OCS — staging canonical payload digest (규칙: ocs-payload-digest/1)
-- 브라우저 로컬 검증 payload 와 서버 staging payload 의 동일성만 대조하기 위한 읽기 전용 함수.
-- OCS 의미·ABD 번호·참조·중복·항등식을 재검사하지 않는다.
...
BEGIN
  ...
  -- responses (confidence_score 는 표현 차이 위험으로 digest 대상 제외)
  WITH l AS (
    SELECT concat_ws(v_us,
      abd_ocs_pd_tok(group_id), abd_ocs_pd_tok(source_parent_comment_id),
  ...
  RETURN jsonb_build_object(
    ...
    'payload_sha256', encode(digest(convert_to(v_txt, 'UTF8'), 'sha256'), 'hex'),
    'groups', v_g,
    'comments', v_c,
    'attachments', v_a,
    'responses', v_r,
