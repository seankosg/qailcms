...
BEGIN
  -- (0) latest_status_norm 계산: CHECK(A/B/C/NS/TERM/NULL) 준수.
  --     원본이 A/B/C 이면 그 값, 아니면 NULL. (Terminated 는 아래에서 별도 처리)
  v_raw_up := CASE WHEN NEW.latest_status IS NULL THEN NULL ELSE upper(btrim(NEW.latest_status)) END;
  NEW.latest_status_norm := CASE
    WHEN v_raw_up IN ('A','B','C') THEN v_raw_up
    ELSE NULL
  END;

  -- (0.5) status_mismatch: 원본이 비어있지 않고 A/B/C 도 아니면 true (U/R, R, P 등)
...
-- 기존 행 백필: 트리거 재실행으로 파생값 재계산
UPDATE public.abd_items_raw SET latest_status = latest_status;
