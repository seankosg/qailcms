...
GRANT EXECUTE ON FUNCTION public.update_task_summary(text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.update_task_summary(text, text) TO service_role;
...
begin
  for r in
    select distinct discipline, task_no
    from public.task_management_raw
    where level = 'parent'
      and task_no is not null
  loop
    perform public.update_task_summary(r.discipline, r.task_no);
  end loop;
end;
$$;
