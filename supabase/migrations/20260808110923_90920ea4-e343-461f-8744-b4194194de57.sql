-- ════════════════════════════════════════════════════════════════
-- WRT · SPL 누락 실적 역산 백필 (일회성, 2026-08-08)
--
-- REVERT (실행하지 말 것):
--   UPDATE public.wrt_stage_progress SET actual_start = NULL, actual_finish = NULL,
--          actual_estimated = false WHERE actual_estimated;
--   UPDATE public.spl_stage_progress SET actual_start = NULL, actual_finish = NULL,
--          actual_estimated = false WHERE actual_estimated;
-- ════════════════════════════════════════════════════════════════

ALTER TABLE public.wrt_stage_progress
  ADD COLUMN IF NOT EXISTS actual_estimated boolean NOT NULL DEFAULT false;
ALTER TABLE public.spl_stage_progress
  ADD COLUMN IF NOT EXISTS actual_estimated boolean NOT NULL DEFAULT false;

-- 카타르 주말(금 dow=5 · 토 dow=6) → 직전 목요일
...
-- 한 단계 역산: 이미 실적/na/Aconex/앵커 이후면 건너뛰고 참조점만 갱신
...
BEGIN
  ...
  IF aconex OR coalesce((s->>'na')::boolean,false) OR (s->>'so')::int >= a_ord THEN
    RETURN NEXT; RETURN;
  END IF;
  ...
  FOR r IN
    ...
    SELECT s.item_id, a.a_ord, a.a_date,
           jsonb_object_agg(s.stage_code, jsonb_build_object(
             'as', s.a_s, 'af', s.a_f, 'na', s.na, 'so', s.sort_order)) AS m
