DROP FUNCTION IF EXISTS public.defect_snag_progress_cells_json(text[], text[], text[], text[], text, date, date, date, text, boolean, text[]);
DROP FUNCTION IF EXISTS public.defect_snag_progress_cells(text[], text[], text[], text[], text, date, date, date, text, boolean, text[]);

CREATE FUNCTION public.defect_snag_progress_cells(
...
AS $function$
  ...
  ju AS (
    SELECT gk, bucket_iso, stage, field, item_id FROM j GROUP BY 1,2,3,4,5
  ),
  im AS (
  ...
  requested_combo AS (
    SELECT
      sum(CASE s.stage
        WHEN 'start' THEN 1 WHEN 'rectified' THEN 2 WHEN 'pre_inspection' THEN 4
        WHEN 'dar_inspection' THEN 8 WHEN 'closure' THEN 16 WHEN 'ho' THEN 32 ELSE 0 END)::int AS m,
  ...
  combos AS (
    ...
    UNION ALL
    SELECT m, combo FROM requested_combo
...
AS $function$
  WITH agg AS (
    ...
    FROM public.defect_snag_progress_cells(_plan_groups, _teams, _room_groups, _group_by,
      _bucket, _range_start, _range_end, _as_of_date, _plan_mode, _include_agg, _buildings, _agg_stages)
  )
...
GRANT EXECUTE ON FUNCTION public.defect_snag_progress_cells_json(text[],text[],text[],text[],text,date,date,date,text,boolean,text[],text[]) TO authenticated, service_role;