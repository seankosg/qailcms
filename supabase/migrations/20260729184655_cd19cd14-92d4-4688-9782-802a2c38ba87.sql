-- ABD Progress: A-1 baseline plan(rn <= v_active) + B AP(Approval) 스테이지 신설
...
AS $function$
  ...
  -- Actual = approval_date. Plan = "현재 승인 전망"(이동 예측형):
  --   미승인 → 현재 라운드(v_active)의 dar_plan (라운드 전진 시 이동이 의도된 동작),
  --   승인   → 승인 라운드(response_result='A')의 dar_plan 으로 고정, 레거시는 NULL.
  -- 주의: A-1(라운드 계획 소급 보존)과 성격이 다르다. 혼동 금지.
  docs AS (
    SELECT b.gk,
           (b.judge->>'active_round')::int AS v_active,
           (b.judge->>'bucket_top')        AS bucket_top,
           b.approval_date AS ap_actual,
           CASE
  ...
  events AS (
    -- 계획(baseline): 도달한 라운드까지 보존 (rn <= v_active). 미도달 라운드 선제 계획 제외.
    SELECT gk,
...
LANGUAGE sql STABLE SET search_path TO 'public'
AS $function$
  -- _round 파라미터는 시그니처 호환을 위해 유지(항상 'all'로 호출).
  WITH base AS (
    SELECT
      ARRAY(
        SELECT COALESCE(NULLIF(TRIM(CASE dim
