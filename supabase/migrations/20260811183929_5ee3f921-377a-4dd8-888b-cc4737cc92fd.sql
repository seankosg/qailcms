...
BEGIN
  ...
  -- 소급 변경 차단 — 이미 시작한 활성 위임에서 허용되는 변경은 end_date 를 오늘 이후로 두는 것 하나뿐이다.
  IF TG_OP = 'UPDATE' AND OLD.status = 'active' THEN
    ...
    IF OLD.start_date <= v_today THEN
      IF NEW.task_raw_id IS DISTINCT FROM OLD.task_raw_id
         OR NEW.from_pic IS DISTINCT FROM OLD.from_pic
         OR NEW.to_pic IS DISTINCT FROM OLD.to_pic
         OR NEW.start_date IS DISTINCT FROM OLD.start_date
         OR NEW.status IS DISTINCT FROM OLD.status THEN
        RAISE EXCEPTION '이미 시작한 위임은 종료일 조정 외에는 바꿀 수 없습니다 (과업·위임자·인수자·시작일·상태 고정).';
      END IF;
