...
declare
  _out jsonb;
begin
  if not public.has_role(auth.uid(), 'system_administrator') then
    raise exception 'org_demob_plan: system administrator only';
  end if;
...
revoke all on function public.org_demob_plan() from public;
grant execute on function public.org_demob_plan() to authenticated;
