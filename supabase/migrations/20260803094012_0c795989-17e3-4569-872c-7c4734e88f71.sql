DROP FUNCTION IF EXISTS public.defect_items_facets(text, text, boolean);
...
begin
  ...
  for _filter in select * from jsonb_array_elements(coalesce(_filters, '[]'::jsonb)) loop
    ...
    if _col = _column then continue; end if; -- exclude self
  ...
  -- 빈 값(null/'')도 '__EMPTY__' 버킷으로 반환해 필터에서 선택 가능하게 한다.
  _sql := format($fmt$
    select v as value, count(*)::bigint as cnt
    from (
      select coalesce(nullif(btrim(%s), ''), '__EMPTY__') as v
