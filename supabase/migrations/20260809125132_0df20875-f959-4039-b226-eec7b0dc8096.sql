-- D-5: SPL latest_status 값에 취소(CX)·종료(TM) 수용
ALTER TABLE public.spl_items DROP CONSTRAINT IF EXISTS spl_items_latest_status_check;
ALTER TABLE public.spl_items ADD CONSTRAINT spl_items_latest_status_check
  CHECK (latest_status = ANY (ARRAY['A','B','C','D','UR','NYS','CX','TM']));

-- WRT Aconex 임포트 적용 함수 (UPDATE 전용; 행 생성 없음)
