-- Add server-side computation of `start_status` to defect_items_search, defect_items_search_ids, and defect_items_facets
-- so the column supports the same sort/filter/facet semantics as rectified_status / closure_status.

-- Reusable case expression (kept identical across the three functions). Uses Asia/Qatar current date
-- to match the client-side todayInDoha() reference the UI uses when computing the badge.

-- ============================================================================
-- 1) defect_items_facets — add start_status support
-- ============================================================================
...
begin
  ...
  if _column = 'start_status' then
    ...
    _sql := format(
      'select %s::text as value, count(*)::bigint as cnt
         from public.defect_items_raw
        where %s and %s is not null
        group by %s
        order by cnt desc, value asc
        limit 500',
      _col_ref, _where, _col_ref, _col_ref
    );
  else
    _sql := format(
...
-- ============================================================================
-- 2) defect_items_search — allow filter/sort on start_status
-- ============================================================================
...
-- ============================================================================
-- 3) defect_items_search_ids — same filter semantics for select-all-matching
-- ============================================================================
