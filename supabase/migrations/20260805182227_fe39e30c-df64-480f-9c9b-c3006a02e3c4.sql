CREATE OR REPLACE FUNCTION public.create_main_with_subs(_discipline text, _main jsonb, _subs jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT MAX(data_date) INTO v_data_date FROM public.task_management_raw WHERE discipline=_discipline;
  IF v_data_date IS NULL THEN
    SELECT MAX(data_date) INTO v_data_date FROM public.task_management_raw;
  END IF;
  IF v_data_date IS NULL THEN
    v_data_date := (now() AT TIME ZONE 'Asia/Qatar')::date;
  END IF;
  ...
  INSERT INTO public.task_management_raw(
    ...
    status_manual, is_rollup, data_date
  ) VALUES (
  ...
  FOR v_sub IN SELECT * FROM jsonb_array_elements(_subs) LOOP
    ...
    INSERT INTO public.task_management_raw(
      ...
      hdec_pic_name, hdec_eng_name, floor_level, location, plot, row_type,
      status_manual, plan_start, plan_end, data_date
    ) VALUES (
