...
ALTER TABLE public.abd_items_raw
  ADD COLUMN IF NOT EXISTS needs_revise boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS revise_source_round smallint;

-- 2) 인덱스
CREATE INDEX IF NOT EXISTS idx_abd_needs_revise
  ON public.abd_items_raw(needs_revise) WHERE needs_revise = true;

-- 3) 트리거 함수 교체 (Revise 판정 추가)
...
BEGIN
  ...
  -- 계획수정필요(Revise): Response 실적일이 다음 라운드 DS Plan을 초과한 경우
  -- (Latest Status=A는 위에서 이미 return, 여기서는 미승인 항목만)
  IF NEW.r1_dar_actual IS NOT NULL
     AND NEW.r2_draft_start_plan IS NOT NULL
     AND NEW.r1_dar_actual > NEW.r2_draft_start_plan
     AND NEW.r1_response_result IN ('B','C') THEN
    v_needs_revise := true;
    v_revise_src := 1;
    v_delay := array_append(v_delay, 'Revise');
  END IF;
  IF NEW.r2_dar_actual IS NOT NULL
     AND NEW.r3_draft_start_plan IS NOT NULL
     AND NEW.r2_dar_actual > NEW.r3_draft_start_plan
     AND NEW.r2_response_result IN ('B','C') THEN
    v_needs_revise := true;
    v_revise_src := 2;
    v_delay := array_append(v_delay, 'Revise');
  END IF;
...
-- 4) 전체 재계산 (트리거를 트리거하기 위한 no-op UPDATE)
