...
AS $function$
  WITH p AS (
    ...
    SELECT
      ...
      count(*) FILTER (
        WHERE d.actual_pre_inspection_date IS NOT NULL AND d.actual_pre_inspection_date <= p.as_of
      )::bigint AS pre_cnt,
      count(*) FILTER (
        WHERE d.actual_dar_inspection_date IS NOT NULL AND d.actual_dar_inspection_date <= p.as_of
      )::bigint AS dar_cnt,
      count(*) FILTER (
        WHERE d.actual_closure_date IS NOT NULL AND d.actual_closure_date <= p.as_of
      )::bigint AS closed_cnt,
      count(*) FILTER (
        WHERE d.actual_ho_date IS NOT NULL AND d.actual_ho_date <= p.as_of
      )::bigint AS ho_cnt
