...
AS $function$
  ...
  -- SB(Submission): _round='all' 일 때 문서 단위로 1회만 집계(재제출 중복 제거).
  --   Actual = 최초 제출 실적일(min), Plan = 활성 라운드 제출 계획일(없으면 r1→r2→r3).
  docs AS (
    SELECT b.id AS item_id,
           (b.judge->>'bucket_top') AS bucket_top,
  ...
  docs2 AS (
    SELECT d.*,
           CASE WHEN d.sb_actual_raw = 'infinity'::date THEN NULL ELSE d.sb_actual_raw END AS sb_actual
    FROM docs d
  )
  SELECT item_id, stage, 'planned'::text, pdate
  FROM expanded
  WHERE pdate IS NOT NULL
    AND NOT (_round = 'all' AND stage = 'submission')
    AND (
  ...
  WHERE adate IS NOT NULL
    AND NOT (_round = 'all' AND stage = 'submission')
  UNION ALL
  SELECT item_id, 'submission'::text, 'planned'::text, sb_plan
  FROM docs2
  WHERE _round = 'all'
    AND sb_plan IS NOT NULL
    AND (_plan_mode = 'baseline' OR sb_actual IS NULL OR sb_actual > _as_of_date)
  UNION ALL
  SELECT item_id, 'submission'::text, 'actual'::text, sb_actual
  FROM docs2
  WHERE _round = 'all'
    AND sb_actual IS NOT NULL
  UNION ALL
