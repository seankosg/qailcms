...
CREATE OR REPLACE FUNCTION public.abd_stage_group_counts(
...
AS $$
  SELECT coalesce(jsonb_agg(jsonb_build_object(
           'stage_group', sg, 'team', team, 'total', total, 'delayed', delayed
         ) ORDER BY sg, team), '[]'::jsonb)
  FROM (
...
GRANT EXECUTE ON FUNCTION public.abd_stage_group(public.abd_items_raw) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.abd_stage_group_counts(text[], text[], text[]) TO authenticated, service_role;
...
BEGIN
  ...
  IF _sg LIKE 'sg~_%' ESCAPE '~' OR _sg LIKE 'sgd~_%' ESCAPE '~' THEN
