CREATE OR REPLACE VIEW public.v_task_management_raw_derived AS
 ...
 SELECT t.id,
    ...
    tm_classify_overdue(t.plan_end, c.target_date, bd.buffer_days) AS plan_overdue,
    tm_expected_finish(t.actual_start, t.actual_finish, t.actual_progress, t.data_date) AS expected_finish,
    tm_classify_overdue(COALESCE(t.actual_finish, tm_expected_finish(t.actual_start, t.actual_finish, t.actual_progress, t.data_date)), c.target_date, bd.buffer_days) AS actual_overdue,
        CASE
