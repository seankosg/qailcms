-- Drop previous row-per-record signatures
...
-- ============================================================
-- 1) tm_items_search — jsonb single-value return
--    returns { rows: [...], total_count: bigint, main_count: bigint }
--    rows: full sub+main records for the page's mains (payload-per-row inside a scalar jsonb)
--    total_count: total row count matching filters
--    main_count: distinct main count matching filters (pagination unit)
-- ============================================================
...
begin
  ...
  _sql := format($fmt$
    ...
    totals as (
      select (select count(*) from filtered)::bigint as total_count,
             (select count(*) from mains_ordered)::bigint as main_count
    ),
    page_mains as (
    ...
    select jsonb_build_object(
      ...
      'main_count', totals.main_count
    )
    from rows_agg cross join totals
...
-- ============================================================
-- 2) tm_items_search_ids — jsonb array of id strings
-- ============================================================
...
begin
  ...
  _sql := format($fmt$
    ...
    from (
      ...
      from public.v_task_management_raw_derived
      where %s
      order by discipline nulls last, coalesce(main_task_no, task_no), sort_order nulls last
      limit %s
    ) t
